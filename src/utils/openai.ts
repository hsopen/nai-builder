...
export async function splitTargetSiteTitle(prompt: string, model = "gpt-3.5-turbo") {
  try {
    const response = await openai.chat.completions.create({
      model,
      messages: [
        { role: "system", content: "我将给你一个网站的标题以及网站的地址，你帮我将网站标题分割为两部分，尽量将品牌名作为主标题，主标题在前，副标题在后，使用<|>分割,只返回非中文标题字符串，不要任何多余的不相关字符，不要为我解释介绍等等" },
 ...
 * @param model 模型，可选，默认 gpt-4
 */
export async function getWebsiteDesc(prompt: string, model = "gpt-3.5-turbo") {
  try {
    const response = await openai.chat.completions.create({
      model,
      messages: [
        {
          role: "system", content: `
我们的谷歌独立站需要写一份英文的description，我写了一份初稿，需要你帮我优化重写。
1、不要改变原有意思，如果初稿中出现了邮箱、电话等联系方式，或者域名品牌需要你删掉；
2、语言风格是美式英语； 
3、不要有明显的AI痕迹； 
4、你只需要输出你写好的description内容即可，不要输出其他任何内容！ 
5、长度和之前一样不要有太多差异        
          ` },
        { role: "user", content: prompt }
      ],
